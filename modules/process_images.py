...
# COLOR EXTRACTION
    ...
    def process_image(self, path, model_type, scale):
        ...
        for i in range(len(bbox)):
            ...
            cv2.putText(data, f"{i+1}", (int(bbox[i][0]), int(bbox[i][1])-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36,255,12), 2)
        
        mask = self.build_mask(masks,bbox)

        scores = [f"{i+1}: {float(round(score, 2))}" for i, score in enumerate(scores)]
